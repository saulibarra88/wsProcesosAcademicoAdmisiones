...
module.exports.ObtenerMallaAcademicaCarreraPesum = async function (carrera,pesum) {
  var sentencia = "";
 // sentencia = "WITH MateriasPensum AS ( SELECT MP.*, P.strCodigo AS strPeriodo, M.strNombre AS strNombreMateria, A.strNombre AS strNombreArea, REPLACE(ISNULL(MPH.strCodMateria, M.strCodigo), '.', '') AS strCodMateriaPensum, ISNULL(MPH.strCodMateria, MP.strCodMateria) AS strCodMateriaLookup, ROW_NUMBER() OVER ( PARTITION BY REPLACE(ISNULL(MPH.strCodMateria, M.strCodigo), '.', ''), MP.strCodNivel ORDER BY MP.strCodMateria ) AS rn FROM [" + carrera + "].[dbo].[Materias_Pensum] AS MP INNER JOIN ( SELECT strCodPensum, MAX(strCodigo) AS strCodigo FROM [" + carrera + "].[dbo].[Periodos] GROUP BY strCodPensum ) AS P ON P.strCodPensum = MP.strCodPensum INNER JOIN [" + carrera + "].[dbo].[Materias] AS M ON M.strCodigo = MP.strCodMateria INNER JOIN [" + carrera + "].[dbo].[Areas] AS A ON A.strCodigo = MP.strCodArea LEFT JOIN [" + carrera + "].[dbo].[Materias_Pensum] AS MPH ON MPH.strCodPensum = MP.strCodPensum AND MPH.strcodPadreIti = MP.strCodMateria AND MPH.strCodTipo = 'ITI' WHERE MP.strCodPensum = '" + pesum + "' AND (MP.strcodPadreIti IS NULL OR MP.strcodPadreIti = 'ITI') ), PrerrequisitosMateria AS ( SELECT MP.strCodMateriaPensum, MP.strNombreMateria, MP.strNombreArea, MP.strPeriodo, MP.strCodPensum, MP.strCodPadreIti, MP.strCodNivel, MP.bytHorasAut, MP.bytHorasPrac, MP.bytHorasSeman, MP.bytHorasTeo, MP.fltCreditos, MP.strCodArea, MP.strCodFormaDict, STUFF(( SELECT '; ' + REPLACE(PR.strCodMatPre, '.', '') FROM [" + carrera + "].[dbo].[Prerrequisitos] PR WHERE PR.strCodMateria = MP.strCodMateriaLookup AND PR.strCodTipo = 'PRE' FOR XML PATH('') ), 1, 2, '') AS strPrerrequisitosCodigos, STUFF(( SELECT '; ' + M2.strNombre FROM [" + carrera + "].[dbo].[Prerrequisitos] PR INNER JOIN [" + carrera + "].[dbo].[Materias] M2 ON M2.strCodigo = PR.strCodMatPre WHERE PR.strCodMateria = MP.strCodMateriaLookup AND PR.strCodTipo = 'PRE' FOR XML PATH('') ), 1, 2, '') AS strPrerrequisitosNombres FROM MateriasPensum MP WHERE MP.rn = 1 ) SELECT * FROM PrerrequisitosMateria ORDER BY strCodNivel, strCodMateriaPensum;"
//  sentencia = "WITH MateriasPensum AS ( SELECT MP.*, P.strCodigo AS strPeriodo, M.strNombre AS strNombreMateria, A.strNombre AS strNombreArea, REPLACE(ISNULL(MPH.strCodMateria, M.strCodigo), '.', '') AS strCodMateriaPensum, ISNULL(MPH.strCodMateria, MP.strCodMateria) AS strCodMateriaLookup, ROW_NUMBER() OVER ( PARTITION BY REPLACE(ISNULL(MPH.strCodMateria, M.strCodigo), '.', ''), MP.strCodNivel ORDER BY MP.strCodMateria ) AS rn FROM [" + carrera + "].[dbo].[Materias_Pensum] AS MP INNER JOIN ( SELECT strCodPensum, MAX(strCodigo) AS strCodigo FROM [" + carrera + "].[dbo].[Periodos] GROUP BY strCodPensum ) AS P ON P.strCodPensum = MP.strCodPensum INNER JOIN [" + carrera + "].[dbo].[Materias] AS M ON M.strCodigo = MP.strCodMateria INNER JOIN [" + carrera + "].[dbo].[Areas] AS A ON A.strCodigo = MP.strCodArea LEFT JOIN [" + carrera + "].[dbo].[Materias_Pensum] AS MPH ON MPH.strCodPensum = MP.strCodPensum AND MPH.strcodPadreIti = MP.strCodMateria AND MPH.strCodTipo = 'ITI' WHERE MP.strCodPensum = '" + pesum + "' AND (MP.strcodPadreIti IS NULL OR MP.strcodPadreIti = 'ITI') ), PrerrequisitosMateria AS ( SELECT MP.strCodMateriaPensum, MP.strNombreMateria, MP.strNombreArea, MP.strPeriodo, MP.strCodPensum, MP.strCodPadreIti, MP.strCodNivel, MP.bytHorasAut, MP.bytHorasPrac, MP.bytHorasSeman, MP.bytHorasTeo, MP.fltCreditos, MP	str CodArea, MP	str CodFormaDict, STUFF(( SELECT '; ' + REPLACE(PR	str CodMatPre, '.', '') FROM [" + carrera + "].[dbo].[Prerrequisitos] PR WHERE PR	str CodMateria = MP	str CodMateriaLookup AND PR	str CodTipo = 'PRE' FOR XML PATH('') ), 1, 2, '') AS strPrerrequisitosCodigos, STUFF(( SELECT '; ' + M2	str Nombre FROM [" + carrera + "].[dbo].[Prerrequisitos] PR INNER JOIN [" + carrera + "].[dbo].[Materias] M2 ON M2	str Codigo = PR	str CodMatPre WHERE PR	str CodMateria = MP	str CodMateriaLookup AND PR	str CodTipo = 'PRE' FOR XML PATH('') ), 1, 2, '') AS strPrerrequisitosNombres FROM MateriasPensum MP WHERE MP.rn = 1 ) SELECT * FROM PrerrequisitosMateria ORDER BY strCodNivel, strCodMateriaPensum;"
sentencia="WITH MateriasPensum AS ( SELECT MP.*, M.strNombre AS strNombreMateria, A.strNombre AS strNombreArea, REPLACE(ISNULL(MPH.strCodMateria, M.strCodigo), '.', '') AS strCodMateriaPensum, ISNULL(MPH.strCodMateria, MP.strCodMateria) AS strCodMateriaLookup, ROW_NUMBER() OVER ( PARTITION BY REPLACE(ISNULL(MPH.strCodMateria, M.strCodigo), '.', ''), MP.strCodNivel ORDER BY MP.strCodMateria ) AS rn FROM [" + carrera + "].[dbo].[Materias_Pensum] AS MP INNER JOIN [" + carrera + "].[dbo].[Materias] AS M ON M.strCodigo = MP.strCodMateria INNER JOIN [" + carrera + "].[dbo].[Areas] AS A ON A.strCodigo = MP.strCodArea LEFT JOIN [" + carrera + "].[dbo].[Materias_Pensum] AS MPH ON MPH.strCodPensum = MP.strCodPensum AND MPH.strcodPadreIti = MP.strCodMateria AND MPH.strCodTipo = 'ITI' WHERE MP.strCodPensum = '" + pesum + "' AND (MP.strcodPadreIti IS NULL OR MP.strcodPadreIti = 'ITI') ), PrerrequisitosMateria AS ( SELECT MP.strCodMateriaPensum, MP.strNombreMateria, MP.strNombreArea, MP.strCodPensum, MP.strCodPadreIti, MP.strCodNivel, MP.bytHorasAut, MP.bytHorasPrac, MP.bytHorasSeman, MP.bytHorasTeo, MP.fltCreditos, MP.strCodArea, MP.strCodFormaDict, STUFF(( SELECT '; ' + REPLACE(PR.strCodMatPre, '.', '') FROM [" + carrera + "].[dbo].[Prerrequisitos] PR WHERE PR.strCodMateria = MP.strCodMateriaLookup AND PR.strCodTipo = 'PRE' FOR XML PATH('') ), 1, 2, '') AS strPrerrequisitosCodigos, STUFF(( SELECT '; ' + M2.strNombre FROM [" + carrera + "].[dbo].[Prerrequisitos] PR INNER JOIN [" + carrera + "].[dbo].[Materias] M2 ON M2.strCodigo = PR.strCodMatPre WHERE PR.strCodMateria = MP.strCodMateriaLookup AND PR.strCodTipo = 'PRE' FOR XML PATH('') ), 1, 2, '') AS strPrerrequisitosNombres FROM MateriasPensum MP WHERE MP.rn = 1 ) SELECT * FROM PrerrequisitosMateria ORDER BY strCodNivel, strCodMateriaPensum;"
  try {
    if (sentencia != "") {
      const sqlConsulta = await execDinamico(carrera, sentencia, "OK", "OK");
      return sendResponseModelo(true, sqlConsulta, 'OK')
...
module.exports.CalculoEstudiantesRegulares60PorCientoAsignaturasActual = async function (carrera, periodo, cedula) {
var sentencia="";
    sentencia="WITH MallaData AS ( SELECT m.strCodPeriodo, m.strCodNivel AS Nivelmatricula, p.strCodPensum, COUNT(*) AS TotalAsignaturaNivelMalla, COUNT(*) * 0.6 AS Asignatura60PorCientoMalla FROM [" + carrera + "].[dbo].[Matriculas] AS m INNER JOIN [" + carrera + "].[dbo].[Periodos] AS p ON p.strCodigo = m.strCodPeriodo INNER JOIN [" + carrera + "].[dbo].[Materias_Pensum] AS mp ON mp.strCodPensum = p.strCodPensum AND mp.strCodNivel = m.strCodNivel INNER JOIN [" + carrera + "].[dbo].[Estudiantes] AS E ON E.strCodigo = m.strCodEstud WHERE m.strCodPeriodo = '" + periodo + "' AND E.strCedula = '" + cedula + "' GROUP BY m.strCodPeriodo, m.strCodNivel, p.strCodPensum ), MatriculadasData AS ( SELECT ma.strCodNivel AS Nivelmatricula, COUNT(*) AS TotalAsignaturasMatriculadas FROM [" + carrera + "].[dbo].[Matriculas] AS m INNER JOIN [" + carrera + "].[dbo].[Estudiantes] AS E ON E.strCodigo = m.strCodEstud INNER JOIN [" + carrera + "].[dbo].[Materias_Asignadas] AS ma ON ma.sintCodMatricula = m.sintCodigo AND ma.strCodPeriodo = m.strCodPeriodo WHERE m.strCodPeriodo = '" + periodo + "' AND ma.strCodPeriodo = '" + periodo + "' AND E.strCedula = '" + cedula + "' GROUP BY ma.strCodNivel ) SELECT MD.strCodPeriodo, MD.nivelmatricula, MD.strCodPensum, MD.TotalAsignaturaNivelMalla AS [TotalMalla], MD.Asignatura60PorCientoMalla AS [60PorcentajeMalla], MatD.TotalAsignaturasMatriculadas AS [TotalMatriculadas], CONVERT(VARCHAR, CAST( CASE WHEN MD.TotalAsignaturaNivelMalla > 0 THEN (MatD.TotalAsignaturasMatriculadas * 100.0 / MD.TotalAsignaturaNivelMalla) ELSE 0 END AS DECIMAL(5,1) ) ) + '%' AS [PorcentajeMatriculado], CASE WHEN ISNULL(MatD.TotalAsignaturasMatriculadas, 0) >= MD.Asignatura60PorCientoMalla THEN 'REGULAR' ELSE 'NO REGULAR' END AS [Estado] FROM MallaData MD LEFT JOIN MatriculadasData MatD ON MD.nivelmatricula = MatD.nivelmatricula; "
  try {
    const sqlConsulta = await execDinamico(carrera, sentencia, "OK", "OK");
    return sendResponseModelo(true, sqlConsulta, 'OK');
  } catch (error) {
...
module.exports.ObtenerNivelMasAltoCarreraPensum = async function (carrera, pensum) {
var sentencia="";
    sentencia="SELECT MAX(strCodNivel) AS MayorNivel FROM [" + carrera + "].[dbo].[Materias_Pensum] WHERE [strCodPensum] = '" + pensum + "'"
  try {
    const sqlConsulta = await execDinamico(carrera, sentencia, "OK", "OK");
    return sendResponseModelo(true, sqlConsulta, 'OK');
  } catch (error) {
