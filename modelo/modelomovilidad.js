...
module.exports.ActualizarDocumentosDadoIdSolicitudTipo = async function (carrera, idSolicitud, idDocumento, tipo, url) {
  var sentencia = "";
  sentencia = " UPDATE [" + carrera + "].[dbo].[tb_movilidad_solicitud_documentos] SET msd_strurl='" + url + "'WHERE msd_id=" + idDocumento + " AND  [msd_idsolicitud]=" + idSolicitud + " AND [msd_estado]=1 AND [msd_strtipo]='" + tipo + "'"
  try {
    if (sentencia != "") {
      const sqlConsulta = await execMaster(carrera, sentencia, "OK", "OK");
      return (sqlConsulta)
...
module.exports.ActualizarEstadoSolicitud = async function (carrera, periodo, cedula, idsolicitud, observacion, estado) {
  var sentencia = "";
  sentencia = "UPDATE [" + carrera + "].[dbo].[tb_movilidad_solicitud] SET [cm_strobservacion]='" + observacion + "',[cm_idtipo_estado]='" + estado + "' WHERE [cm_periodo]='" + periodo + "' AND [cm_id]=" + idsolicitud + " AND [cm_identificacion]='" + cedula + "'"
  try {
    if (sentencia != "") {
      const sqlConsulta = await execMaster(carrera, sentencia, "OK", "OK");
      return (sqlConsulta)
