...
router.get('/LisatdoDictadoAsignaturaCarreraDocentes/:dbcarrera/:periodo',async (req, res) => {
    const dbcarrera = req.params.dbcarrera;
    const periodo = req.params.periodo;
    try {
        var Informacion=await  procesosmoodlefunciones.ProcesoModleMigracionDatosDictadoCarreraDocentes(dbcarrera,periodo);
        if (Informacion.proceso) {
            return sendResponseServicios(res, true, Informacion.datos,'OK');
        }
        else {
